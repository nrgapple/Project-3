...
{
  memoryManager vmmi = memoryManager(POLICY, BITS_PER_BLOCK, NUMBER_OF_PAGES,
                                     VIRT_ADDRESS_SPACE);
  ...
  cin >> operations;
  cout << "Input the average place in virtual memory where you want requests" <<
       " to occur[0, ]: ";
  ...
  cauchy_distribution<double> distribution(averageVirtLoc, stDeviation);
  ...
  for (int i = 0; i < operations; i++) {
    ...
    if (!PRINT_IN_HEX)
      cout << "Virtual [" << number << "] is in Physical[" << whatsReturned <<
           "] " << (currSwaps != vmmi.numberPageSwaps() ? "SWAPPED" : "") <<
           endl;
    else
      cout << "Virtual [" << std::hex << number << "] is in Physical[" <<
           std::hex << whatsReturned << "] " <<
           (currSwaps != vmmi.numberPageSwaps() ? "SWAPPED" : "") << endl;
